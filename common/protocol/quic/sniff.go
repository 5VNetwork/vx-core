...
import (
	...
	ptls "github.com/5vnetwork/vx-core/common/protocol/tls"
)
...
// b can contain multiple QUIC packets concatenated
func SniffQUIC(b []byte) (*SniffHeader, error) {
	if len(b) == 0 {
		return nil, protocol.ErrNoClue
	}

	// avoid modify the original buffer
	newB := bytespool.Alloc(int32(len(b)))
	defer bytespool.Free(newB)
	n := copy(newB, b)
	b = newB[:n]
	...
	// zero the buffer. If not, nouce will be very random, and it can cause
	// cipher.Open failed with "cipher: message authentication failed"
	cryptoDataBuf.ZeroAll()
	...
	for len(b) > 0 {
		...
		if len(b) < hdrLen+int(packetLen) {
			return nil, protocol.ErrNoClue // Not enough data to read as a QUIC packet. QUIC is UDP-based, so this is unlikely to happen.
		}
