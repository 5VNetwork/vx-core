...
func (w *MuxWriter) WriteMultiBuffer(mb buf.MultiBuffer) error {
	...
	for !mb.IsEmpty() {
		...
		if w.transferType == TransferTypeStream {
...
func (w *MuxWriter) writeData(mb buf.MultiBuffer) error {
...
func (w *MuxWriter) getNextFrameMeta() FrameMetadata {
	meta := FrameMetadata{
		SessionID: w.id,
		Target:    w.dest,
	}
	...
	if w.followup {
		...
		// meta.UdpUuid = w.udpUuid
	}

	return meta
}

func (w *MuxWriter) writeMetaOnly() error {
	meta := w.getNextFrameMeta()
	b := buf.New()
