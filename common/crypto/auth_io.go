...
type AuthenticationReaderIO struct {
	auth         Authenticator
	r            io.Reader
	reader       *bufio.Reader
...
func NewAuthenticationReaderIO(auth Authenticator, sizeParser ChunkSizeDecoder, reader io.Reader,
	transferType protocol.TransferType, paddingLen PaddingLengthGenerator) *AuthenticationReaderIO {
	r := &AuthenticationReaderIO{
		auth:         auth,
		r:            reader,
...
func (r *AuthenticationReaderIO) readSize() (uint16, uint16, error) {
	if r.hasSize {
		r.hasSize = false
...
func (r *AuthenticationReaderIO) readBuffer(size int32, padding int32) (*buf.Buffer, error) {
	b := buf.New()
...
func (r *AuthenticationReaderIO) readInternal(soft bool, mb *buf.MultiBuffer) error {
...
func (r *AuthenticationReaderIO) Read(p []byte) (int, error) {
...
func (r *AuthenticationReaderIO) Close() error {
