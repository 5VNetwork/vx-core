...
func (t Type) New(pattern string) (Matcher, error) {
	...
	case Prefix:
		return PrefixMatcher(pattern), nil
	default:
...
func ToDomain(pattern string) (string, error) {
	for {
		...
		for i := 0; i < len(pattern); i++ {
			...
			case c == '.':
			default:
				return "", fmt.Errorf("pattern string does not conform to Letter-Digit-Hyphen (LDH) subset, %s", pattern)
			}
		}
		if !isASCII {
			var err error
