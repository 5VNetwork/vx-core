...
type trieNode0 struct {
	values   []uint32
	children map[string]*trieNode0
}
...
func (g *DomainMatcherGroup) Build() error {
	// No build needed for trie
	return nil
}
...
func (g *DomainMatcherGroup) AddDomainMatcher(matcher DomainMatcher, value uint32) {
	...
	for i := len(pattern); i > 0; {
		...
		if node.children == nil {
			node.children = make(map[string]*trieNode0)
		}
