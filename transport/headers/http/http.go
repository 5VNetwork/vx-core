...
func (h *HeaderReader) Read(reader io.Reader) (*buf.Buffer, error) {
	...
	for totalBytes < maxHeaderLength {
		...
		if n := bytes.Index(buffer.Bytes(), []byte(ENDING)); n != -1 {
			...
			buffer.AdvanceStart(int32(n + len(ENDING)))
			endingDetected = true
			break
		}
...
func init() {
	common.Must(creator.RegisterConfig((*Config)(nil), func(ctx context.Context, config interface{}) (interface{}, error) {
		return NewAuthenticator(config.(*Config))
	}))
}
