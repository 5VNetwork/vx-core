...
import (
	...
	"github.com/5vnetwork/vx-core/common/signal/semaphore"
)
...
func NewConnection(meta ConnMetadata, writer PacketWriter, closer io.Closer, config *KcpConfig) *Connection {
	// errors.New("#", meta.Conversation, " creating connection to ", meta.RemoteAddr).WriteToLog()
...
func (c *Connection) writeMultiBufferInternal(reader io.Reader) error {
	...
	for {
		for {
			...
			if b == nil {
				b = buf.New()
				_, err := b.ReadOnce(io.LimitReader(reader, int64(c.mss)))
				if err != nil {
					return nil
				}
			}
...
func (c *Connection) SetState(state State) {
	...
	// errors.New("#", c.meta.Conversation, " entering state ", state, " at ", current).AtDebug().WriteToLog()
...
func (c *Connection) Close() error {
	...
	// errors.New("#", c.meta.Conversation, " closing connection to ", c.meta.RemoteAddr).WriteToLog()
...
func (c *Connection) Terminate() {
	...
	// errors.New("#", c.meta.Conversation, " terminating connection to ", c.RemoteAddr()).WriteToLog()
...
func (c *Connection) flush() {
	...
	if c.State() == StateTerminating {
		// errors.New("#", c.meta.Conversation, " sending terminating cmd.").AtDebug().WriteToLog()
