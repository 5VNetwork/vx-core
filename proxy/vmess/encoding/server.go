...
import (
	...
	vmessaead "github.com/5vnetwork/vx-core/proxy/vmess/aead"
)
...
func (h *SessionHistory) removeExpiredEntries() error {
	...
	if len(h.cache) == 0 {
		return errors.New("nothing to do", nil)
	}
...
func (s *ServerSession) DecodeRequestHeader(reader io.Reader, drainOnError bool) (*protocol.RequestHeader, error) {
	buffer := buf.New()

	drainer, err := drain.NewBehaviorSeedLimitedDrainer(int64(s.userValidator.GetBehaviorSeed()), 16+38, 3266, 64)
	if err != nil {
		return nil, errors.New("failed to initialize drainer", err)
	}
	...
	if drainOnError {
		...
		if _, err := buffer.ReadOnceWithSize(reader, protocol.IDBytesLen); err != nil {
			return nil, fmt.Errorf("failed to read request header: %w", err)
		}
		if buffer.Len() != protocol.IDBytesLen {
			return nil, fmt.Errorf("failed to read request header: expected %d bytes, got %d", protocol.IDBytesLen, buffer.Len())
		}
	}
	...
	case foundAEAD:
		...
		if errorReason != nil {
			if shouldDrain {
				...
				return nil, drainConnection(errors.New("AEAD read failed", errorReason))
			}
			return nil, drainConnection(errors.New("AEAD read failed, drain skipped", errorReason))
		}
	...
	case errorAEAD == vmessaead.ErrNotFound:
		...
		if !valid || userValidationError != nil {
			return nil, drainConnection(errors.New("invalid user", userValidationError))
		}
		if s.isAEADForced {
			return nil, drainConnection(errors.New("invalid user: VMessAEAD is enforced and a non VMessAEAD connection is received. You can still disable this security feature with environment variable v2ray.vmess.aead.forced = false . You will not be able to enable legacy header workaround in the future."))
		}
		if s.userValidator.ShouldShowLegacyWarn() {
			log.Warn().Msg("Critical Warning: potentially invalid user: a non VMessAEAD connection is received. From 2022 Jan 1st, this kind of connection will be rejected by default. You should update or replace your client software now. This message will not be shown for further violation on this inbound.")
		}
		user = userLegacy
		iv := hashTimestamp(md5.New(), timestamp)
		vmessAccount = userLegacy

		aesStream := crypto.NewAesDecryptionStream(vmessAccount.ID.CmdKey(), iv)
		decryptor = crypto.NewCryptionReader(aesStream, reader)

	default:
		return nil, drainConnection(errors.New("invalid user", errorAEAD))
	}
	...
	if !s.sessionHistory.addIfNotExits(sid) {
		if !s.isAEADRequest {
			...
			if drainErr != nil {
				return nil, drainConnection(errors.New("duplicated session id, possibly under replay attack, and failed to taint userHash", drainErr))
			}
			return nil, drainConnection(errors.New("duplicated session id, possibly under replay attack, userHash tainted"))
		}
	...
	case protocol.RequestCommandTCP, protocol.RequestCommandUDP:
		...
		// if special && request.Command == vmess.RequestCommandUDP {
		// 	_, err = buffer.ReadFullFrom(decryptor, 16)
		// 	if err != nil {
		// 		return nil, fmt.Errorf("failed to read UDP uuid, %w", err)
		// 	}
		// 	if uuid, err := uuid.ParseBytes(buffer.BytesRange(-16, buffer.Len())); err == nil {
		// 		request.UdpUuid = uuid
		// 	}
		// }
	}
	...
	if paddingLen > 0 {
		if _, err := buffer.ReadFullFrom(decryptor, int32(paddingLen)); err != nil {
			if !s.isAEADRequest {
				...
				if burnErr != nil {
					return nil, fmt.Errorf("failed to read padding, failed to taint userHash %w %w", burnErr, err)

				}
				return nil, fmt.Errorf("failed to read padding, userHash tainted %w", err)
			}
			return nil, errors.New("failed to read padding", err)
		}
	}
	...
	if _, err := buffer.ReadFullFrom(decryptor, 4); err != nil {
		if !s.isAEADRequest {
			burnErr := s.userValidator.BurnTaintFuse(fixedSizeAuthID[:])
			if burnErr != nil {
				return nil, fmt.Errorf("failed to read checksum, failed to taint userHash %w %w", burnErr, err)
			}
			return nil, fmt.Errorf("failed to read checksum, userHash tainted %w", err)
		}
		return nil, errors.New("failed to read checksum", err)
	}
	...
	if actualHash != expectedHash {
		if !s.isAEADRequest {
			Autherr := errors.New("invalid auth, legacy userHash tainted")
			burnErr := s.userValidator.BurnTaintFuse(fixedSizeAuthID[:])
			if burnErr != nil {
				Autherr = errors.New("invalid auth, can't taint legacy userHash", burnErr)
			}
	...
	if request.Security == protocol.SecurityType_UNKNOWN || request.Security == protocol.SecurityType_AUTO {
		return nil, fmt.Errorf("unknown security type: %v", request.Security)
	}
...
func (s *ServerSession) DecodeRequestBody(ctx context.Context, request *protocol.RequestHeader, reader io.Reader) (buf.Reader, error) {
	...
	case protocol.SecurityType_NONE:
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationReader(ctx, auth, sizeParser, reader, protocol.TransferTypePacket, padding), nil
		}
	...
	case protocol.SecurityType_LEGACY:
		...
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationReader(ctx, auth, sizeParser, cryptionReader, request.Command.TransferType(), padding), nil
		}
	...
	case protocol.SecurityType_AES128_GCM:
		...
		return crypto.NewAuthenticationReader(ctx, auth, sizeParser, reader, request.Command.TransferType(), padding), nil
...
// DecodeRequestBody returns Reader from which caller can fetch decrypted body.
func (s *ServerSession) DecodeRequestBody1(ctx context.Context, request *protocol.RequestHeader, reader io.Reader) (io.Reader, error) {
	var sizeParser crypto.ChunkSizeDecoder = crypto.PlainChunkSizeParser{}
	if request.Option.Has(protocol.RequestOptionChunkMasking) {
		sizeParser = NewShakeSizeParser(s.requestBodyIV[:])
	}
	...
	case protocol.SecurityType_NONE:
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			if request.Command.TransferType() == protocol.TransferTypeStream {
				return crypto.NewChunkStreamReader1(sizeParser, reader, 0), nil
			}
	...
	case protocol.SecurityType_LEGACY:
		...
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationReaderIO(auth, sizeParser, cryptionReader, request.Command.TransferType(), padding), nil
		}
...
func (s *ServerSession) EncodeResponseBody1(request *protocol.RequestHeader, writer io.Writer) (io.Writer, error) {
	var sizeParser crypto.ChunkSizeEncoder = crypto.PlainChunkSizeParser{}
	if request.Option.Has(protocol.RequestOptionChunkMasking) {
		sizeParser = NewShakeSizeParser(s.responseBodyIV[:])
	}
	...
	case protocol.SecurityType_NONE:
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationWriterIO(auth, sizeParser, writer, protocol.TransferTypePacket, padding), nil
		}
		return writer, nil
	...
	case protocol.SecurityType_LEGACY:
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationWriterIO(auth, sizeParser, s.responseWriter, request.Command.TransferType(), padding), nil
		}
		return &buf.SequentialWriter{Writer: s.responseWriter}, nil
	...
	case protocol.SecurityType_AES128_GCM:
		...
		return crypto.NewAuthenticationWriterIO(auth, sizeParser, writer, request.Command.TransferType(), padding), nil
