...
func NewClientSession(ctx context.Context, isAEAD bool, idHash protocol.IDHash, behaviorSeed int64) *ClientSession {
	...
	}
	{
		...
		if err != nil {
			log.Ctx(ctx).Error().Msg("unable to initialize drainer")
...
func (c *ClientSession) EncodeRequestHeader(header *protocol.RequestHeader, writer io.Writer) error {
	...
	account := header.Account.(*vmess.MemoryAccount)
	if !c.isAEAD {
	...
	if header.Command != protocol.RequestCommandMux {
		if err := addrParser.WriteAddressPort(buffer, header.Address, header.Port); err != nil {
			return errors.New("failed to writer address and port", err)
		}
	}
...
func (c *ClientSession) EncodeRequestBody(request *protocol.RequestHeader, writer io.Writer) (buf.Writer, error) {
	...
	if request.Option.Has(protocol.RequestOptionGlobalPadding) {
		...
		if !ok {
			return nil, errors.New("invalid option: RequestOptionGlobalPadding", nil)
		}
	}
...
func (c *ClientSession) EncodeRequestBody1(request *protocol.RequestHeader, writer io.Writer) (io.Writer, error) {
	...
	case protocol.SecurityType_LEGACY:
		...
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationWriterIO(auth, sizeParser, cryptionWriter, request.Command.TransferType(), padding), nil
		}
...
func (c *ClientSession) DecodeResponseHeader(reader io.Reader) (*protocol.ResponseHeader, error) {
	if !c.isAEAD {
		...
		if n, err := io.ReadFull(reader, aeadEncryptedResponseHeaderLength[:]); err != nil {
			...
			return nil, drain.WithError(c.readDrainer, reader, errors.New("Unable to Read Header Len", err))
		} else { // nolint: revive
			c.readDrainer.AcknowledgeReceive(n)
		}
		if decryptedResponseHeaderLengthBinaryBuffer, err := aeadResponseHeaderLengthEncryptionAEAD.Open(nil, aeadResponseHeaderLengthEncryptionIV, aeadEncryptedResponseHeaderLength[:], nil); err != nil {
			return nil, drain.WithError(c.readDrainer, reader, errors.New("Failed To Decrypt Length", err))
		} else { // nolint: revive
		...
		if n, err := io.ReadFull(reader, encryptedResponseHeaderBuffer); err != nil {
			c.readDrainer.AcknowledgeReceive(n)
			return nil, drain.WithError(c.readDrainer, reader, errors.New("Unable to Read Header Data", err))
		} else { // nolint: revive
			c.readDrainer.AcknowledgeReceive(n)
		}
		...
		if decryptedResponseHeaderBuffer, err := aeadResponseHeaderPayloadEncryptionAEAD.Open(nil, aeadResponseHeaderPayloadEncryptionIV, encryptedResponseHeaderBuffer, nil); err != nil {
			return nil, drain.WithError(c.readDrainer, reader, errors.New("Failed To Decrypt Payload", err))
		} else { // nolint: revive
	...
	if _, err := buffer.ReadFullFrom(c.responseReader, 4); err != nil {
		return nil, errors.New("failed to read response header", err)
	}

	if buffer.Byte(0) != c.responseHeader {
		return nil, fmt.Errorf("unexpected response header. Expecting %d but actually %d", int(c.responseHeader), int(buffer.Byte(0)))
	}
	...
	if buffer.Byte(2) != 0 {
		...
		if _, err := buffer.ReadFullFrom(c.responseReader, dataLen); err != nil {
			return nil, errors.New("failed to read response command", err)
		}
...
func (c *ClientSession) DecodeResponseBody(ctx context.Context, request *protocol.RequestHeader, reader io.Reader) (buf.Reader, error) {
	...
	case protocol.SecurityType_LEGACY:
		if request.Option.Has(protocol.RequestOptionChunkStream) {
			...
			return crypto.NewAuthenticationReader(ctx, auth, sizeParser, c.responseReader, request.Command.TransferType(), padding), nil
		}
...
func (c *ClientSession) DecodeResponseBody1(request *protocol.RequestHeader, reader io.Reader) (io.Reader, error) {
	var sizeParser crypto.ChunkSizeDecoder = crypto.PlainChunkSizeParser{}
	if request.Option.Has(protocol.RequestOptionChunkMasking) {
		sizeParser = NewShakeSizeParser(c.responseBodyIV[:])
	}
