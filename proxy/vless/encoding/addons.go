...
func EncodeBodyAddons(writer io.Writer, request *protocol.RequestHeader, requestAddons *Addons, state *vless.TrafficState, context context.Context) buf.Writer {
	if request.Command == protocol.RequestCommandUDP {
		return buf.NewMultiLengthPacketWriter(writer.(buf.Writer))
	}
	w := buf.NewWriter(writer)
	if requestAddons.Flow == vless.XRV {
		w = vless.NewVisionWriter(w, state, context)
	}
	return w
}
