...
import (
	...
	"github.com/5vnetwork/vx-core/proxy/wireguard/gvisortun"
	...
	if handler != nil {
		...
		tcpForwarder := tcp.NewForwarder(stack, 0, 65535, func(r *tcp.ForwarderRequest) {
			go func(r *tcp.ForwarderRequest) {
				...
				if err != nil {
					log.Error().Msgf("failed to create tcp endpoint: %v", err)
					r.Complete(true)
					return
				}
				r.Complete(false)
		...
		udpForwarder := udp.NewForwarder(stack, func(r *udp.ForwarderRequest) bool {
			go func(r *udp.ForwarderRequest) {
				...
				ep, err := r.CreateEndpoint(&wq)
				if err != nil {
					log.Error().Msgf("failed to create udp endpoint: %v", err)
					return
				}
				defer ep.Close()
